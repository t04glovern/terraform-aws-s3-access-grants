...
def index():
    if oidc.user_loggedin:
        return 'Welcome, {}! <a href="/logout">Log out</a>'.format(oidc.user_getfield('email'))
    else:
...
def get_s3_data():
    ...
    decoded_token = jwt.decode(aws_token['idToken'], options={"verify_signature": False})
    ...
    s3_object = s3_client.get_object(Bucket=os.getenv('AWS_TARGET_BUCKET_NAME'), Key='users/user_list')
